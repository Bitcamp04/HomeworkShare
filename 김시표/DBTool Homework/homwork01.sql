...
-- 조회된 여러개의 칼럼의 데이터 셋 하나로 인식해서
-- 조회된 모든 컬럼의 데이터가 중복돼야 중복으로 인식한다.
SELECT DISTINCT JOB
	 , MGR
	FROM EMP;
-- JOB,MGR 둘다 중복일때만 잡는단 얘기
...
-- 6-4. 칼럼에 별칭을 붙인 경우에는 별칭으로도 정렬가능
SELECT ENO
	 , ENAME
	 , DNO AS 부서번호
	 , SAL AS 급여
...
--	WHERE TO_CHAR(HDATE) < '2000-10-10:23:59:59';   --부적절한 쿼리
	
	WHERE HDATE < TO_CHAR('2000-10-10:23:59:59', 'YYYY-MM-DD:HH24:MI:SS');   -- 적절한 쿼리
...
-- 보너스가 NULLDK DKSLS 사원의 사원번호, 사원이름, 업무 급여 보너스 조회
SELECT ENO
	 , ENAME
	 , JOB
	 , SAL
...
-- 8-3. AND, OR 혼합 사용
-- AND, OR을 혼합하여 사용하면 조건을 알아보기 힘들어지기 때문에
-- () 조건의 우선순위를 정해준다.
-- 전공이 화학과이거나 물리인 학생 중 1학년 학생의 학생번호, 학생이름, 전공, 학년 조회
SELECT SNO
	 , SNAME
	 , MAJOR
	 , SYEAR
...
-- 전공이 화학과이거나 물리인 학생의 학생번호, 학생이름, 전공, 학년 조회
SELECT SNO
	 , SNAME
	 , MAJOR
	 , SYEAR
...
-- 큰 따옴표는 ("")는 컬럼 별칭(alias)를 붙여줄 때만 사용한다.
