...
INSERT INTO SCORE_CHK VALUES ('2','1',55);
INSERT INTO SCORE_CHK VALUES ('B','A',70);
INSERT INTO SCORE_CHK VALUES ('C','1',99);
...
CREATE TABLE COURSE_CONTSRAINT (
	...
	CONSTRAINT FK_COURSE_CONTSRAINT_PNO FOREIGN KEY(PNO) REFERENCES PROFESSOR_PK(PNO)
	);
...
BEGIN
	...
	SELECT C.CNO,C.CNAME,P.PNO,P.PNAME INTO CNO,CNAME,PNO,PNAME 
	FROM COURSE C
	JOIN PROFESSOR P
	ON C.PNO = P.PNO
	WHERE C.CNAME='유기화학';

	DBMS_OUTPUT.PUT_LINE('과목 번호'||CNO||'과목이름'||CNAME||'교수 번호'||PNO||'교수이름'||PNAME);
EXCEPTION 
...
BEGIN
	...
	SELECT C.CNO,C.CNAME,P.PNO,P.PNAME INTO CPREC 
	FROM COURSE C
	JOIN PROFESSOR P
	ON C.PNO = P.PNO
	WHERE C.CNAME='유기화학';

	DBMS_OUTPUT.PUT_LINE('과목 번호'||CPREC.CNO||'과목이름'||CPREC.CNAME||'교수 번호'||CPREC.PNO||'교수이름'||CPREC.PNAME);
EXCEPTION 
	WHEN NO_DATA_FOUND THEN 
	DBMS_OUTPUT.PUT_LINE('해당 데이터를 찾지 못했습니다.');
END;
